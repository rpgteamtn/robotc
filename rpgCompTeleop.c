...
#define CATCHUP 255 //Degree measure for when goal capture is released
#define CATCHDOWN 0 //Degree measure for when goal capture is down
#define BUCKETup 45 //Degree measure for when the bucket is up
#define BUCKETdown 45 //Degree measure for when bucket is released
...
task main()
{
	...
	{getJoystickSettings(joystick);  // Update Buttons and Joysticks
		...
		{
			int iCRate = servoChangeRate[goalCapture];	// Save change rate
			servoChangeRate[bucketServo] = 0; 					// Max Speed
			servo[bucketServo] = BUCKETdown;					    // Set servo position
			wait1Msec(20);
			servoChangeRate[bucketServo] = iCRate;			// Reset the servo
		}
		else if(joy1Btn(JOY_BUTTON_LT))
		{
			int iCRate = servoChangeRate[goalCapture];	// Save change rate
			servoChangeRate[bucketServo] = 0; 					// Max Speed
			servo[bucketServo] = BUCKETup;					      // Set servo position
			wait1Msec(20);
			servoChangeRate[bucketServo] = iCRate;			// Reset the servo
		}
